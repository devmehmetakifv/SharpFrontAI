...
namespace SharpFrontAI
{
    public class WebSection
    {
        ...
        public async Task CodeAsync(GeminiAI AI,Prompt prompt)
        {
            string url = $"https://generativelanguage.googleapis.com/v1beta/models/{AI.Model}:generateContent?key={AI.APIKey}";
            string json = $@"{{
                ""contents"": [{{
                    ""parts"": [{{
                        ""text"": ""{prompt.Content}""
